...
function RawMaterialsPaymentOut(props) {
    ...
    const [rawmatpendinglist, setRawMaterialsPurchasePendingList] = useState([]);
    const [rawmatpendinglistfilter, setRawMaterialsPurchasePendingListFilter] = useState([]);
    ...
    useEffect((e) => {
        if (localStorage.getItem('token')) {
            ...
            fetchPendingRawMaterialsInvoiceList(parseInt(paymentDetails.CustomerId || 0));
    ...
    const supplierOnChange = (e) => {
        ...
        history('/rawmaterialspaymentout/?custtype=' + customertype + '&uid=' + e.target.value);
        addCount(count);
    }
    ...
    const settleAdvancePayment = async (e) => {
        ...
        e.preventDefault();
        setIsLoaded(true);
        for (const p of rawmatpendinglist) {
            if (newAdvVal !== 0) {
                if (parseFloat(p.Due) < parseFloat(newAdvVal)) {
                    const response = await fetch(process.env.REACT_APP_API
                        + 'RawMaterials/RawMaterialsInvoicePaymentUpdate', {
                        method: 'PUT',
                        headers: {
                            'Accept': 'application/json',
                            'Content-Type': 'application/json',
    ...
    const fetchPendingRawMaterialsInvoiceList = async (uid) => {
        setIsPendingListLoaded(false);
        fetch(process.env.REACT_APP_API + 'RawMaterials/GetPendingRawMaterialsInvoiceList?CustId='
            ...
            .then(data => {
                if (data.StatusCode === 200) {
                    setRawMaterialsPurchasePendingList(data.Result);
                    setRawMaterialsPurchasePendingListFilter(data.Result);
                ...
                <div class="col-8">
                    {
                        advancedata != null && advancedata.Amount > 0 && <div className="row justify-content-center">
                            <div className="alert alert-success" role="alert">
                                ...
                                {
                                    rawmatpendinglist && rawmatpendinglist.length > 0 &&
...
export default RawMaterialsPaymentOut
