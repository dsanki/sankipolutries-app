...
function FeedPaymentOut(props) {
     ...
     let history = useNavigate();
        ...
        const [feedpendinglist, setFeedPurchasePendingList] = useState([]);
        const [feedpendinglistfilter, setFeedPurchasePendingListFilter] = useState([]);
        ...
        useEffect(() => {
         ...
         useEffect((e) => {
                if (localStorage.getItem('token')) {
                    ...
                    GetPendingFeedInvoiceList(parseInt(paymentDetails.CustomerId || 0));
                    fetchFeedPaymentOutHistory(parseInt(paymentDetails.CustomerId || 0));
                }
                else {
                    HandleLogout();
                    history("/login")
            ...
            const supplierOnChange = (e) => {
                ...
                history('/chicksfeedpaymentout/?custtype=' + customertype + '&uid=' + e.target.value);
                addCount(count);
            }
            ...
            const settleAdvancePayment = async (e) => {
                ...
                e.preventDefault();
                setIsLoaded(true);
                for (const p of feedpendinglist) {
                    if (newAdvVal !== 0) {
                        if (parseFloat(p.Due) < parseFloat(newAdvVal)) {
                            const response = await fetch(process.env.REACT_APP_API
                                + 'Feed/FeedInvoicePaymentUpdate', {
                                method: 'PUT',
                                headers: {
                                    'Accept': 'application/json',
                                    'Content-Type': 'application/json',
                        ...
                        if (newVal !== 0) {
                            if (parseFloat(p.Due) < parseFloat(newVal)) {
                                ...
                                const todo = await response.json()
                                    .then((result) => {
                                        if (result.StatusCode === 200) {
                                            newVal = parseFloat(newVal) - parseFloat(p.Due);
                                        }
                                    });
                            }
                            else {
            ...
            const GetPendingFeedInvoiceList = async (uid) => {
                setIsPendingListLoaded(false);
                fetch(process.env.REACT_APP_API + 'Feed/GetPendingFeedInvoiceList?CustId='
                    ...
                    .then(data => {
                        if (data.StatusCode === 200) {
                            setFeedPurchasePendingList(data.Result);
                            setFeedPurchasePendingListFilter(data.Result);
                        ...
                        <div class="col-8">
                            {
                                advancedata != null && advancedata.Amount > 0 && <div className="row justify-content-center">
                                    <div className="alert alert-success" role="alert">
                                        ...
                                        {
                                            feedpendinglist && feedpendinglist.length > 0 &&
        ...
        feedpendinglist && feedpendinglist.length > 0 ? feedpendinglist.map((p) => {
                                                    return (
